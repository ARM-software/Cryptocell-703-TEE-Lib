...
typedef enum HwCryptoKey {
	USER_KEY = 0,			/* 0b0000 */
	ROOT_KEY = 1,			/* 0b0001 */
	SESSION_KEY = 3,		/* 0b0011 */
	RESERVED_KEY = 4,		/* NA */
	PLATFORM_KEY = 5,		/* 0b0101 */
	CUSTOMER_KEY = 6,		/* 0b0110 */
	KFDE0_KEY = 7,			/* 0b0111 */
	KFDE1_KEY = 9,			/* 0b1001 */
	KFDE2_KEY = 10,			/* 0b1010 */
	KFDE3_KEY = 11,			/* 0b1011 */
	KPICV_KEY = 13, 	    /* 0b1101 */ /* ==KPICV */
	KCP_KEY = 15,	        /* 0b1111 */ /* ==KCP */
...
/*!
 * This macro signals that we are asking for Kplt calculation
 *
 * \param pDesc pointer HW descriptor struct
 */
#define HW_DESC_SET_ACK_KPLT_CALC(pDesc) 									\
	do {												\
		CC_REG_FLD_SET(CRY_KERNEL, DSCRPTR_QUEUE_WORD4, ACK_NEEDED, (pDesc)->word[4], 3);	\
	} while (0)
...
#define HW_DESC_SET_DIN_TYPE(pDesc, dmaMode, dinAdr, dinSize, axiNs)								\
	do {		                                                                                        		\
		...
		CC_REG_FLD_SET(CRY_KERNEL, DSCRPTR_QUEUE_WORD5, DIN_ADDR_HIGH, (pDesc)->word[5], ((dinAdr)>>32)&UINT16_MAX );	\
...
#define HW_DESC_SET_DOUT_TYPE(pDesc, dmaMode, doutAdr, doutSize, axiNs)							\
	do {		                                                                                        	\
		...
		CC_REG_FLD_SET(CRY_KERNEL, DSCRPTR_QUEUE_WORD5, DOUT_ADDR_HIGH, (pDesc)->word[5], ((doutAdr)>>32)&UINT16_MAX );	\
 ...
 * \param axiNs AXI secure bit
 */
#define HW_DESC_SET_DOUT_DLLI(pDesc, doutAdr, doutSize, axiNs)								\
                HW_DESC_SET_DOUT_TYPE(pDesc, DMA_DLLI, doutAdr, doutSize, axiNs)		                                                                                        		\
 ...
 * \param axiNs AXI secure bit
 */
#define HW_DESC_SET_DOUT_MLLI(pDesc, doutAdr, doutSize, axiNs)								\
                HW_DESC_SET_DOUT_TYPE(pDesc, DMA_MLLI, doutAdr, doutSize, axiNs)		                                                                                        		\
 ...
 */
#define HW_DESC_SET_DOUT_NO_DMA(pDesc, doutAdr, doutSize)							\
	do {		                                                                                        		\
...
/*!
 * This macro selects the SM3 engine instead of HASH engine
 *
 * \param pDesc pointer HW descriptor struct
 */
#define HW_DESC_SET_SM3_MODE(pDesc)                                        \
    do {                                                                                                    \
        CC_REG_FLD_SET(CRY_KERNEL, DSCRPTR_QUEUE_WORD4, AES_XOR_CRYPTO_KEY, (pDesc)->word[4], 1);           \
    } while (0)
