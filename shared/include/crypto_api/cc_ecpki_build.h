...
values, using CC_EcpkiGetDomain() or CC_EcpkiGetSm2Domain() functions.

 @return CC_OK on success.
 @return A non-zero value on failure as defined cc_ecpki_error.h.
*/
CIMPORT_C CCError_t CC_EcpkiPrivKeyBuild(
					/*! [in] The EC domain (curve). */
				     const CCEcpkiDomain_t *pDomain,
					 /*! [in] Pointer to private key data. */
				     const uint8_t         *pPrivKeyIn,
					 /*! [in] Size of private key data (in bytes). */
				     size_t                 PrivKeySizeInBytes,
					 /*! [out] Pointer to the private key structure. This structure is used as input to the ECPKI cryptographic primitives. */
...
CIMPORT_C CCError_t CC_EcpkiPublKeyBuildAndCheck(
			...
			/*! [in]  Pointer to the input public key data, in compressed or
										   ...
										   compression. </li></ul>*/
		    uint8_t                     *pPubKeyIn,
			/*! [in]  The size of public key data (in bytes). */
		    size_t                      PublKeySizeInBytes,
			/*! [in]  The required level of public key verification
		    ...
		    ECPublKeyCheckMode_t        CheckMode,
             /*! [out] Pointer to the output public key structure.
										This structure is used as input to the ECPKI cryptographic primitives. */
		    CCEcpkiUserPublKey_t        *pUserPublKey,
			/*! [in]  Pointer for a temporary buffer required for the build function. */
