...
/*!
 ...
 @details
 * Using Sign/Verify API is straightforward,
 * just call Sign/Verify functions and provide the message hash that should be
 ...
 *
 * <li>Party A should call to CC_Sm2CalculateECPoint() and send the
 ...
 *
 * <li>Each party may call the CC_Sm2Confirmation() function (if confirmation value was used in the previous steps).</li>
 *
...
typedef struct CC_Sm2KeContext_t {
    ...
    int                         isInitiator;
    /*! First bit encodes weather this party wants confirmation, second bit encodes the confirmation for other party
     * for example 3 for both parts, 1 - only this party wants confirmation, 2 - only the other party wants confirmation.*/
...
It takes as an input the message digest as a little endian words that come
...
CIMPORT_C CCError_t CC_Sm2Sign(
        ...
        const size_t                    HashInputSize,     /*!< [in]      The size of message data hash in words. */
...
CIMPORT_C CCError_t CC_Sm2Verify (
        ...
        const size_t                    HashInputSize          /*!< [in/out] The size of the hash of the input data (in words). */
);
...
CIMPORT_C CCError_t CC_Sm2KeyExchangeContext_init(
        CC_Sm2KeContext_t               *pCtx,              /*!< [in/out]    This pointer should be allocated by user. This function inits it.*/
        uint8_t                         *pWorkingBuffer,    /*!< [in]        The working buffer */
        const size_t                    wblen,              /*!< [in]        The working buffer size should be at least
        ...
        uint8_t                         is_initiator,       /*!< [in]        1 if it is an initiator side. */
        uint8_t                         conf_required       /*!< [in]        bit mask - 1st bit if we want conf, 2nd if the other part wants*/
);
