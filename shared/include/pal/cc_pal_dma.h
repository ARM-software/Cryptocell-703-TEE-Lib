...
/*!
 @brief User buffer scatter information. */
typedef struct {
...
uint32_t CC_PalDmaBufferMap(
    ...
    /*! [in/out] <ul><li> In - The maximal number of blocks to fill.</li><li>Out - the actual number of blocks.</li></ul> */
    uint32_t                     *pNumOfBlocks,
    /*! [out] The list of DMA-able blocks that the buffer maps to. */
    CCPalDmaBlockInfo_t          *pDmaBlockList,
    /*! [out] A handle to the private resources of the mapped buffer.*/
...
uint32_t CC_PalDmaBufferUnmap(
    /*! [in] The address of the buffer to unmap. */
    uint8_t                       *pDataBuffer,
    /*! [in] The buffer size in Bytes. */
    uint32_t                       buffSize,
    /*! [in] The copy direction of the buffer, according to ::CCPalDmaBufferDirection_t:
    ...
    FROM_DEVICE.</li></ul> */
    CCPalDmaBufferDirection_t      copyDirection,
    /*! [in] The number of DMA-able blocks that the buffer maps to. */
    uint32_t                       numOfBlocks,
    /*! [in] The list of DMA-able blocks that the buffer maps to. */
    CCPalDmaBlockInfo_t            *pDmaBlockList,
    /*! [in] A handle to the private resources of the mapped buffer. */
    CC_PalDmaBufferHandle          dmaBuffHandle  );
 ...
 * @return A non-zero value in case of failure.
 */
uint32_t CC_PalDmaContigBufferAllocate(
    /*! [in] The buffer size in Bytes.*/
    uint32_t          buffSize,
    /*! [out]  The virtual address of the allocated buffer.*/
 ...
 * @return A non-zero value in case of failure.
 */
uint32_t CC_PalDmaContigBufferFree(
    /*! [in] The buffer size in Bytes. */
    uint32_t          buffSize,
    /*! [in] The virtual address of the buffer to free. */
 ...
 */
uint32_t CC_PalIsDmaBufferContiguous(
    /*! [in] The address of the user buffer. */
    uint8_t                     *pDataBuffer,
    /*! [in] The size of the user buffer. */
    uint32_t                    buffSize   );
