...
enum drv_crypto_alg {
    ...
    DRV_CRYPTO_ALG_SM3 = 7,
    DRV_CRYPTO_ALG_SM4 = 8,
    DRV_CRYPTO_ALG_NUM = 9,
...
struct drv_ctx_hash {
    ...
    DRV_HASH_LENGTH_WORDS - CC_DIGEST_SIZE_MAX / sizeof(uint32_t)
                    - CC_HMAC_BLOCK_SIZE_MAX / sizeof(uint32_t)];
};
...
struct drv_ctx_cipher {
    ...
    CC_AES_BLOCK_SIZE / sizeof(uint32_t) - 2 * (CC_AES_KEY_SIZE_MAX / sizeof(uint32_t))];
};
