...
/**
 ...
 @param [in,out] p_rng  - Pointer to the random context - the input to f_rng.
                                           ...
                                           CCEcpkiKgCertContext_t      *pFipsCtx)            /*in*/
{
        ...
        *  negligible low probability of its random generation                */
        ...
        err = CC_RndGenerateVectorInRange(f_rng, p_rng, (uint32_t)pDomain->ordSizeInBits,
        ...
        if (pFipsCtx != NULL) {
                CC_PalMemSetZero(pFipsCtx, sizeof(CCEcpkiKgCertContext_t));
        }
