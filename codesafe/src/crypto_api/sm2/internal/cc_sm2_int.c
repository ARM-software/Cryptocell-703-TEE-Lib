...
)
{
    ...
    size_t                      ordSizeInBytes = 0;
    const CCEcpkiDomain_t*      pDomain = CC_EcpkiGetSm2Domain(); /* Currently the standard specifies only one possible domain for SM2. */
    uint8_t                     working_buffer[2 * CC_SM3_RESULT_SIZE_IN_BYTES + 2 * CC_SM2_ORDER_LENGTH_IN_BYTES + 4];
    size_t                      wblen = 2 * CC_SM3_RESULT_SIZE_IN_BYTES + 2 * CC_SM2_ORDER_LENGTH_IN_BYTES + 4;
    ...
    working_buffer[wblen-1] = ct;
    CC_CommonReverseMemcpy      ( working_buffer,                   (uint8_t*)pSm2KeContext->V.x,       ordSizeInBytes );
    CC_CommonReverseMemcpy      ( working_buffer + ordSizeInBytes,  (uint8_t*)pSm2KeContext->V.y,       ordSizeInBytes );

    if (pSm2KeContext->isInitiator) {
        CC_PalMemCopy               ( working_buffer + ordSizeInBytes*2,          pSm2KeContext->Z,         pSm2KeContext->Z_value_size );
        CC_PalMemCopy               ( working_buffer + ordSizeInBytes*2
                                          + pSm2KeContext->Z_value_size,          pSm2KeContext->Z_remote,  pSm2KeContext->Z_value_size );
    }
    else {
        CC_PalMemCopy               ( working_buffer + ordSizeInBytes*2,          pSm2KeContext->Z_remote,  pSm2KeContext->Z_value_size );
        CC_PalMemCopy               ( working_buffer + ordSizeInBytes*2
                                          + pSm2KeContext->Z_value_size,          pSm2KeContext->Z,         pSm2KeContext->Z_value_size );
    }
