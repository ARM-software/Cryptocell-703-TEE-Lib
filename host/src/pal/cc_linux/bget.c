...
#define BH(p)	((struct bhead *) ((void *)(p)))
...
#define BDH(p)	((struct bdhead *) ((void *)(p)))
...
#define BFH(p)	((struct bfhead *) ((void *)(p)))
...
#define SizeQ	((SizeQuant > (long)QLSize) ? SizeQuant : (long)QLSize)
  ...
  bufsize requested_size;
{
    ...
    * the requested_size is zero and it's valid 			      */
	...
	while (b != &freelist) {
		...
		if ((b->bh.bsize - size) > (SizeQ + (long)(sizeof(struct bhead)))) {
		    struct bhead *ba, *bn;
    ...
    if (acqfcn != NULL) {
	if (size > exp_incr - (long)sizeof(struct bhead)) {
  ...
  void *buf;
{
    ...
    if (relfcn != NULL &&
	((bufsize) b->bh.bsize) == (pool_len - (long)sizeof(struct bhead))) {
